...
class TokenAuthenticator extends AbstractGuardAuthenticator  {
    ...
    public function getCredentials(Request $request) {
        ...
        if($request->headers->has($this::ACCESS_TOKEN_HEADER_NAMESPACE)) {
            $accessToken = $request->headers->get($this::ACCESS_TOKEN_HEADER_NAMESPACE);

            if($request->headers->has($this::GHOST_TOKEN_HEADER_NAMESPACE)) {
                $ghostToken = $request->headers->get($this::GHOST_TOKEN_HEADER_NAMESPACE);

                return array(
                  $this::ACCESS_TOKEN_HEADER_NAMESPACE => $accessToken,
                  $this::GHOST_TOKEN_HEADER_NAMESPACE => $ghostToken
                );
            }
    ...
    public function getUser($credentials, UserProviderInterface $userProvider) {
        ...
        $accessToken = $this->entityManager->getRepository(Token::class)
          ->findOneBy(array('code' => $credentials[$this::ACCESS_TOKEN_HEADER_NAMESPACE]));

        if (!$accessToken
          || !$accessToken->getOwner()
          || !$accessToken->getOwner()->getIsActive()) {
        ...
        if (array_key_exists($this::GHOST_TOKEN_HEADER_NAMESPACE, $credentials) && $accessToken != null) {
            /** @var TokenRepository $tokenRepository */
            $tokenRepository = $this->entityManager->getRepository(Token::class);

            /** @var Token $ghostToken */
            $ghostToken = $tokenRepository->findOneBy(array('code' => $credentials[$this::GHOST_TOKEN_HEADER_NAMESPACE]));

            if (!$ghostToken
              || $ghostToken->getType() != TokenType::GHOST
              || !$ghostToken->getIsVerified()) {
