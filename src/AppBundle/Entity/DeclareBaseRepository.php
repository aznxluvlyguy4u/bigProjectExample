...
class DeclareBaseRepository extends BaseRepository implements DeclareBaseRepositoryInterface
{
     ...
     * @return array
     */
    public function getLog(Animal $animal, $replacementString = '')
    {
        $results = [];
        //null check
        ...
        $sql = "SELECT b.log_date, start_date, end_date, 'MATE' as action, NULL as data, p.first_name, p.last_name
                ...
                WHERE (b.request_state = 'FINISHED' OR b.request_state = 'FINISHED_WITH_WARNING') AND (d.stud_ewe_id = ".$animalId." OR d.stud_ram_id = ".$animalId.")
                UNION
                ...
                WHERE (b.request_state = 'FINISHED' OR b.request_state = 'FINISHED_WITH_WARNING') AND (l.animal_father_id = ".$animalId." OR l.animal_mother_id = ".$animalId.")
                UNION
                ...
                FROM declare_weight d
                  INNER JOIN declare_nsfo_base b ON d.id = b.id
                  LEFT JOIN person p ON p.id = b.action_by_id
                WHERE (b.request_state = 'FINISHED' OR b.request_state = 'FINISHED_WITH_WARNING') AND d.animal_id = ".$animalId."
                UNION
                ...
                FROM declare_arrival a
                  ...
                  LEFT JOIN person p ON p.id = b.action_by_id
                WHERE (b.request_state = 'FINISHED' OR b.request_state = 'FINISHED_WITH_WARNING') AND a.animal_id = ".$animalId."
                UNION
